...
sudo mysql -e "ALTER USER '${DB_USRNAME}'@'localhost' IDENTIFIED WITH mysql_native_password BY '${DB_PASSWORD}';FLUSH PRIVILEGES;"
...
"DB_URL=${DB_URL}" | sudo tee -a /etc/environment
"DB_USERNAME=${DB_USRNAME}" | sudo tee -a /etc/environment
"DB_PASSWORD=${DB_PASSWORD}" | sudo tee -a /etc/environment
